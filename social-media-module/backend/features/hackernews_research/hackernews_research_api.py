#!/usr/bin/env python3
"""
Hacker News Research REST API
Provides endpoints for managing HN research configurations and jobs
"""
...
from features.hackernews_research.hackernews_research_config import (
    AnalysisDepth,
    HackerNewsConfig,
    HackerNewsResearchConfig,
    HackerNewsResearchConfigManager,
    HNStoryType,
    ResearchFrequency,
    ResearchSchedule,
)

app = FastAPI(title="Hacker News Research API", version="1.0.0")
...
config_manager = HackerNewsResearchConfigManager()


# Pydantic models for API requests/responses
class HackerNewsConfigRequest(BaseModel):
    story_types: List[str] = Field(..., example=["top", "ask", "show"])
    search_topics: List[str] = Field(..., example=["AI", "startup", "technology"])
    max_stories_per_type: int = Field(15, ge=1, le=50)
    max_comments_per_story: int = Field(30, ge=0, le=100)
    min_score: int = Field(10, ge=0)
    min_comments: int = Field(5, ge=0)
    comment_depth: int = Field(3, ge=1, le=5)
...
class HackerNewsResearchConfigRequest(BaseModel):
    user_id: str = Field(..., example="user_001")
    workspace_id: str = Field(..., example="workspace_001")
    config_name: str = Field(..., example="tech_trends")
    description: str = Field(..., example="Daily HN research on technology trends")

    hackernews_config: HackerNewsConfigRequest
...
async def root():
    """API health check"""
    return {
        "message": "Hacker News Research API",
        "version": "1.0.0",
        "status": "healthy",
...
async def create_research_config(request: HackerNewsResearchConfigRequest):
    """Create new HN research configuration"""
    try:
        # Convert request to config object
        hn_config = HackerNewsConfig(
            story_types=[HNStoryType(t) for t in request.hackernews_config.story_types],
            search_topics=request.hackernews_config.search_topics,
            max_stories_per_type=request.hackernews_config.max_stories_per_type,
            max_comments_per_story=request.hackernews_config.max_comments_per_story,
            min_score=request.hackernews_config.min_score,
            min_comments=request.hackernews_config.min_comments,
            comment_depth=request.hackernews_config.comment_depth,
            exclude_jobs=request.hackernews_config.exclude_jobs,
            time_range_hours=request.hackernews_config.time_range_hours,
        )
        ...
        config = HackerNewsResearchConfig(
            user_id=request.user_id,
            workspace_id=request.workspace_id,
            config_name=request.config_name,
            description=request.description,
            hackernews_config=hn_config,
        ...
        if success:
            return {
                "success": True,
                "message": f"HN research configuration '{request.config_name}' created successfully",
...
async def trigger_research_job(request: JobTriggerRequest, background_tasks: BackgroundTasks):
    """Manually trigger HN research job"""
    try:
        job_id = f"hn_job_{int(datetime.now().timestamp())}"
        ...
        return {
            "success": True,
            "job_id": job_id,
            "message": f"HN {request.job_type} job started",
...
async def execute_research_job(job_id: str, job_type: str, config_name: Optional[str]):
    """Execute HN research job in background"""
    ...
    try:
        ...
        runner_script = script_dir / "run_hackernews_background.sh"
...
async def trigger_direct_research(request: Dict[str, Any], background_tasks: BackgroundTasks):
    ...
    try:
        job_id = f"hn_direct_{int(datetime.now().timestamp())}"
        ...
        return {
            "success": True,
            "job_id": job_id,
            "message": f"HN {job_type} job started",
            "results_url": f"/jobs/status/{job_id}",
            "metrics": {"estimated_duration": "5-10 minutes", "job_type": job_type},
        }
...
async def health_check():
    ...
    health_status = {
        "service": "hackernews_research_api",
        "status": "healthy",
        "timestamp": datetime.now().isoformat(),

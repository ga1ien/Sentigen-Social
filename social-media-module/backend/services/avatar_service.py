...
from models.avatar_models import (
    ...
    AvatarType, VideoStatus, AspectRatio, SubscriptionTier,
    HeyGenAvatarData, HeyGenTalkingPhotoData, HeyGenVoiceData,
    UserVideoLimits
)
...
class AvatarService:
    ...
    async def get_avatar_profiles(
        ...
        try:
            query = self.db_client.service_client.table("avatar_profiles").select("*").eq(
    ...
    async def create_avatar_profile(
        ...
        try:
            ...
            result = await self.db_client.service_client.table("avatar_profiles").insert(
                profile_data
            ).execute()
    ...
    async def update_avatar_profile(
        ...
        **updates
        ...
        try:
            ...
            result = await self.db_client.service_client.table("avatar_profiles").update(
                updates
            ).eq("id", profile_id).eq("workspace_id", workspace_id).execute()
    ...
    async def delete_avatar_profile(self, profile_id: int, workspace_id: str) -> bool:
        ...
        try:
            await self.db_client.service_client.table("avatar_profiles").delete().eq(
                "id", profile_id
    ...
    async def get_default_avatar_profile(self, workspace_id: str) -> Optional[AvatarProfile]:
        ...
        try:
            result = await self.db_client.service_client.table("avatar_profiles").select("*").eq(
                "workspace_id", workspace_id
    ...
    async def generate_script(
        ...
        try:
            ...
            prompt = self._build_script_prompt(
                topic, target_audience, video_style, duration_target, additional_context
            )
            ...
            result = await self.db_client.service_client.table("script_generations").insert(
                script_data
            ).execute()
    ...
    async def get_user_scripts(
        ...
        workspace_id: str,
        limit: int = 20
    ) -> List[ScriptGeneration]:
        """Get user's generated scripts"""
        try:
            result = await self.db_client.service_client.table("script_generations").select("*").eq(
                "user_id", user_id
        ...
        try:
            ...
            video_data = {
                ...
                "metadata": {
                    ...
                    "heygen_response": heygen_result
                }
            }
            
            result = await self.db_client.service_client.table("video_generations").insert(
                video_data
            ).execute()
    ...
    async def get_video_status(self, video_id: int, user_id: str) -> Optional[VideoGeneration]:
        ...
        try:
            result = await self.db_client.service_client.table("video_generations").select("*").eq(
                "id", video_id
    ...
    async def get_user_videos(
        ...
        try:
            query = self.db_client.service_client.table("video_generations").select("*").eq(
                "user_id", user_id
            ).eq("workspace_id", workspace_id).order("created_at", desc=True).limit(limit)
    ...
    async def sync_heygen_avatars(self, workspace_id: str) -> Dict[str, int]:
        ...
        try:
            ...
            for avatar in avatars:
                result = await self._sync_avatar_profile(
                    workspace_id, avatar, AvatarType.AVATAR
                )
                if result == "created":
                    created_count += 1
                elif result == "updated":
            ...
            for photo in talking_photos:
                result = await self._sync_avatar_profile(
                    workspace_id, photo, AvatarType.TALKING_PHOTO
                )
                if result == "created":
                    created_count += 1
                elif result == "updated":
    ...
    async def get_user_video_limits(self, user_id: str, workspace_id: str) -> UserVideoLimits:
        ...
        try:
            ...
            result = await self.db_client.service_client.table("video_generations").select(
                "id", count="exact"
            ).eq("user_id", user_id).eq("workspace_id", workspace_id).gte(
    ...
    async def _unset_default_avatars(self, workspace_id: str):
        ...
        await self.db_client.service_client.table("avatar_profiles").update({
            "is_default": False
    ...
    async def _get_next_display_order(self, workspace_id: str) -> int:
        ...
        result = await self.db_client.service_client.table("avatar_profiles").select(
            "display_order"
        ).eq("workspace_id", workspace_id).order("display_order", desc=True).limit(1).execute()
    ...
    async def _sync_avatar_profile(
        ...
        try:
            ...
            if result.data:
                ...
                await self.db_client.service_client.table("avatar_profiles").insert(
                    profile_data
                ).execute()
                return "created"

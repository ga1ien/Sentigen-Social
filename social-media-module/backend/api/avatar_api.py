...
from models_avatar import (
    ...
    VideoStatusResponse, AvatarListResponse,
    UserVideoLimits
)
...
async def create_avatar_profile(
    request: CreateAvatarProfileRequest,
    workspace_id: str = "default_workspace"  # TODO: Get from auth
):
...
async def create_bulk_videos(
    ...
    try:
        ...
        if len(request.script_ids) > limits.remaining_videos and not limits.is_admin:
            raise HTTPException(
                status_code=400,
                detail=f"Insufficient video credits. You have {limits.remaining_videos} remaining."
            )
...
async def _create_bulk_videos_background(
    ...
    try:
        ...
        logger.info("Bulk video creation completed", 
                   script_count=len(script_ids), user_id=user_id)

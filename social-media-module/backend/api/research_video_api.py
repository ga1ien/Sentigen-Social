...
class StartWorkflowRequest(BaseModel):
    ...
    research_topics: List[str] = Field(
        ..., description="Topics to research (e.g., ['AI automation', 'productivity tools'])", min_items=1, max_items=5
    )
    platforms_to_research: List[str] = Field(
        default=["reddit", "twitter", "linkedin"], description="Platforms to research for content"
    )
    target_audience: str = Field(
        ..., description="Target audience for the video (e.g., 'tech entrepreneurs', 'developers')"
    )
    video_style: str = Field(default="professional", description="Video style: professional, casual, educational")
    avatar_id: Optional[str] = Field(None, description="HeyGen avatar ID to use")
    voice_id: Optional[str] = Field(None, description="HeyGen voice ID to use")
    max_video_duration: int = Field(default=60, description="Maximum video duration in seconds", ge=15, le=180)
    auto_publish: bool = Field(default=False, description="Auto-publish without approval (use with caution)")
    tiktok_hashtags: Optional[List[str]] = Field(None, description="Custom hashtags for TikTok post")
...
async def start_research_video_workflow(
    ...
    try:
        ...
        if not platforms:
            raise HTTPException(status_code=400, detail="At least one valid platform must be specified")
        ...
        background_tasks.add_task(_execute_workflow_background, config, user_id, workspace_id)

        # Generate workflow ID for tracking
        workflow_id = f"rtv_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"

        logger.info("Research-to-video workflow started", workflow_id=workflow_id, topics=request.research_topics)
...
async def get_pending_approvals(user_id: str = "default_user"):  # TODO: Get from auth
...
async def approve_workflow(workflow_id: str, request: ApprovalRequest):
    ...
    try:
        result = await get_workflow_orchestrator().approve_and_publish(
            workflow_id=workflow_id, approved=request.approved, user_feedback=request.feedback
        )
...
async def list_workflows(
    user_id: str = "default_user", status: Optional[str] = None, limit: int = 20  # TODO: Get from auth
):
...
async def cancel_workflow(workflow_id: str):
    ...
    try:
        ...
        await get_workflow_orchestrator()._update_workflow_status(workflow_id, WorkflowStatus.CANCELLED)
...
async def health_check():
    ...
    try:
        # Check HeyGen connection via worker health check if available
        try:
            heygen_healthy = await get_workflow_orchestrator().video_worker.health_check()
        except Exception:
            heygen_healthy = False
...
async def _execute_workflow_background(config: WorkflowConfig, user_id: str, workspace_id: str):
    """Execute workflow in background"""
    try:
        result = await get_workflow_orchestrator().execute_workflow(
            config=config, user_id=user_id, workspace_id=workspace_id
        )

        logger.info("Background workflow completed", workflow_id=result.workflow_id, status=result.status.value)
